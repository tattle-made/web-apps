...
import { Link, ExternalLink } from "@bit/tattle-tech.core-ui.links";
...
const SmallFooter = ({ items }) => (
  <Box direction={"row"} wrap={true}>
    <Box direction={"row"} pad={"small"} gap={"small"} wrap={true}>
      {items.primary.map((primaryItem) => {
        return primaryItem.type === "internal" ? (
          <Link key={primaryItem.id} to={`/khoj/${primaryItem.target}`}>
    ...
    <Box direction={"row"} pad={"small"} gap={"small"} align={"center"}>
      ...
      <ExternalLink href={"https://github.com/tattle-made"} target={"_blank"}>
        <GitHub size={16} />
      </ExternalLink>
      <ExternalLink href={"https://twitter.com/tattlemade"} target={"_blank"}>
        <Twitter size={16} />
      </ExternalLink>
    </Box>
  </Box>
