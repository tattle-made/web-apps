    ...
    <Grommet theme={TattleTheme} full>
    	<Tabs justify="start">
	      	<Tab title="tab 1" alignSelf="start">
		    	<EditableText data={currentData.textData} updateCallback={updateData} />
		  		<EditableDateRange data={currentData.dateRangeData} updateCallback={updateData} />
		    	<Button margin={{'left':'medium'}} label="Sync" onClick={() => {syncIt(tabElems)}} />
		  	</Tab>
		  	<Tab title="tab 2">
		    	<EditableDate data={currentData.dateData} updateCallback={updateData} />
		  	</Tab>
	  	</Tabs>
    </Grommet>
    )
...
export const EnumMetadata = () => {
    ...
    return (
        <Grommet theme={TattleTheme} full>
            <EditableEnum data={currentData} options={enumOptions} updateCallback={updateData} />
        </Grommet>
    )
};
